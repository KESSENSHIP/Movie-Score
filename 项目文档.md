...
│   ├── public/                    # 静态资源
│   └── package.json
└── movie_analysiz_01/            # Python 数据分析项目
    ├── test01/                    # 第一版分析脚本
    ├── test02/                    # 第二版分析脚本
    └── .venv/                     # Python 虚拟环境
`
...
- 功能: 电影CRUD/搜索/筛选
| GET / | 分页列表 | GET /{id} | 详情 |
| GET /search | 搜索 | GET /filter | 多条件筛选 |
| POST / | 新增 | PUT / | 更新 | DELETE /{id} | 删除 |
...
| PageResult<T> | data, currentPage, pageSize, totalCount, totalPages | 通用分页结果 |
...
| 类 | 路径 | 说明 |
|----|------|------|
| Result<T> | common/Result.java | 统一响应封装(code/message/data/timestamp) |
...
**算法**: Alternating Least Squares (交替最小二乘法)
**实现位置**: movie_analysiz_01/test01/output/als_recommendation.py
**参数**: n_factors=30, n_iterations=15, reg_param=0.1, top_n=20
**流程**:
1. 从MySQL加载评分数据(过滤低频用户<5条)
2. 构建用户-电影评分稀疏矩阵
3. ALS训练: 交替固定一项因子求解另一项因子
4. 保存模型: user_factors.bin, item_factors.bin(flOat32二进制)
5. 保存映射: user_to_idx.json, idx_to_user.json, idx_to_movie.json
6. 为所有用户生成Top-N推荐
...
**算法**: K-Means++聚类(Spark MLlib + Apache Commons Math3双实现)
**特征**: avgRating(平均评分), ratingCount(评分数), ratingStddev(标准差), daysSinceLastRating(活跃天数)
**流程**:
1. 提取用户评分特征(聚合rating表数据)
2. Z-Score标准化特征
3. 快速肘部法则确定最优K(2-10范围)
4. K-Means++聚类(欧氏距离, 30次迭代)
5. 分配用户到簇, 批量保存到user_cluster表
6. 生成簇汇总统计(用户数/平均评分数/标准差)
...
**算法**: ARIMA(p,d,q)自回归积分滑动平均模型
**实现位置**: movie_analysiz_01/test01/output/trend_forecast.py
**流程**:
1. Spark读取电影数据, 按年份统计电影数量
2. 根据数据量选择ARIMA阶数(数据点<10用(1,1,0), 否则(5,1,0))
3. statsmodels拟合ARIMA模型
4. 预测未来3年电影数量
5. 保存为trend_forecast.json供后端读取

### 8.4 DeepSeek大模型情感分析

**API**: DeepSeek Chat API (deepseek-chat模型)
**实现位置**: SentimentAnalysisService + sentiment_analysis.py
**提示词策略**:
...
### 12.2 数据库准备
1. 创建数据库: CREATE DATABASE movie_db CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
2. 导入原始数据 movies.csv, users.csv, ratings.csv, comments.csv, person.csv
3. 运行Python预处理脚本生成干净数据
4. 将数据导入MySQL对应表

### 12.3 后端部署
1. 配置数据源: 修改application.properties中的数据库连接
2. 配置DeepSeek API Key
3. 运行ALS训练脚本: python als_recommendation.py
4. 运行ARIMA预测脚本: python trend_forecast.py
5. 构建项目: mvn clean package
6. 启动服务: java -jar movie-api-0.0.1-SNAPSHOT.jar
7. 服务启动在8888端口, 自动创建所需数据表

### 12.4 前端部署
1. 安装依赖: npm install
2. 配置API地址: 修改vite.config.js中的proxy
3. 开发模式: npm run dev
4. 生产构建: npm run build
5. 预览: npm run preview
6. 构建产物在dist/目录, 可部署到Nginx等静态服务器
...
本项目不仅是一个完整的管理系统，更是一个集成了多种机器学习算法的智能分析平台，
展示了从数据采集、预处理、模型训练到在线服务的完整数据科学工程实践。
对于学习全栈开发、机器学习工程化应用具有很高的参考价值。
